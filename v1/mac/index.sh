...
# matches, and dynamically highlights the best candidates. It also performs a
# lookup for a corresponding detailed procedure based on the top match.
#
...
#
# - Procedure Lookup: The script now looks for a `procedure_id` in the top search
#   result and uses it to find and display a detailed, multi-line procedure
#   from a separate `procedures` directory. This separates searchable metadata
#   from actionable, human-readable instructions.
#
...
# Define the directories for your files.
...
results=$(find "$DATABASE_DIR" -maxdepth 1 -name "*.csv" -print0 | xargs -0 -P 4 -I {} \
...
if [[ -z "$results" ]]; then
    ...
    while IFS='|' read -r result_score matched_keywords CSV_FILE result_row; do
        ...
        for i in "${!headers[@]}"; do
          ...
          if [[ "$header_name" == "procedure_id" ]]; then
            procedure_id="$value"
            if [[ -z "$first_procedure_id" ]]; then
              first_procedure_id="$procedure_id"
            fi
          fi
    ...
    # Pause and prompt before displaying the procedure for the top candidate.
    if [[ -n "$first_procedure_id" ]]; then
      read -p "Press Enter to view the procedure or any other key to exit..." -r -s
      echo

      if [[ -z "$REPLY" ]]; then
        procedure_text=$(awk -F ';' -v id="$first_procedure_id" '
          {
            if ($1 == id) {
              sub(/^[^;]*;/, "");
              gsub(/^"|"$/, "");
        ...
        if [[ -n "$procedure_text" ]]; then
            echo ""
            echo "### ✅ Procedure Found: $first_procedure_id ###"
            echo ""
            printf "%b\n" "$procedure_text"
            echo "-------------------------------------"
        fi
